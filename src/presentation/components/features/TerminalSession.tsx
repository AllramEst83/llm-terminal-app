...
export const TerminalSession: React.FC<TerminalSessionProps> = ({
  ...
  useEffect(() => {
    ...
    const initSession = async () => {
      const loadedSettings = await new ManageSettingsUseCase(sessionId).loadSettings();
  ...
  useEffect(() => {
    ...
    const saveSettings = async () => {
      ...
      await settingsUseCase.saveSettings(settings, { applyTheme: false });
  ...
  // Apply theme when active or theme changes
  useEffect(() => {
    if (!isActive) {
      return;
    }
  ...
  }, [settings.modelName, booted, sessionId]);
  ...
  useEffect(() => {
    if (!isActive) return;
    scrollToBottom();
  }, [messages, isStreaming, bootSequence, isLoading, scrollToBottom, isActive]);
    ...
    while (shouldContinue && iterationCount < MAX_ITERATIONS) {
      ...
      if (CommandService.isCommand(trimmedInput)) {
        ...
        try {
          ...
          // Execute command
          setIsLoading(true);
          const commandUseCase = new HandleCommandUseCase(settings, isStudioEnv, sessionId);
          ...
          if (result.settingsUpdate) {
            const settingsUseCase = new ManageSettingsUseCase(sessionId);
            const updatedSettings = await settingsUseCase.updateSettings(
              settings,
            ...
            if (result.settingsUpdate.apiKey) {
              onApiKeySubmit(result.settingsUpdate.apiKey);
            }
          }
  ...
  }, [isProcessingQueue, settings, isStudioEnv, handleSelectKey, sessionId, onApiKeySubmit]);
